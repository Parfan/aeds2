...
class GameLista {
    ...
    GameLista() {
        this.gameList = new Game[0];
        this.length = this.gameList.length;
    }

    GameLista(Game[] gameList) {
    ...
    void inserir(Game game, int pos) {
        ...
        for (int i = 0; i < this.length - 1; i++) {
            if (i < pos)
                tmp[i] = this.gameList[i];
            else
                tmp[i + 1] = this.gameList[i];
        }
        this.gameList = tmp;
        tmp = null;
    }
    ...
    void inserirFim(Game game) {
        this.inserir(game, this.length);
    }

    void remover(int pos) {
        this.gameList[pos] = null;
        Game[] tmp = new Game[--this.length];
        for (int i = 0; i < this.length; i++) {
            if (i < pos)
                tmp[i] = this.gameList[i];
            else
                tmp[i] = this.gameList[i + 1];
        }
        this.gameList = tmp;
        tmp = null;
    }
    ...
    void imprimir() {
        MyIO.print("[");
        if (this.length > 0)
            MyIO.print(this.gameList[0].getName());
        for (int i = 1; i < this.length; i++)
            MyIO.print(", " + this.gameList[i].getName());
        MyIO.println("]");
    }

    Game get(int pos) {
        return this.gameList[pos];
    }

    void set(Game game, int pos) {
        this.gameList[pos] = game;
    }
}
