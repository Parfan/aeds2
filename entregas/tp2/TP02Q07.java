...
public class TP02Q07 {
    public static void main(String[] args) {
        ...
        Game game = new Game();
        GameFilaCircular gameQueue = new GameFilaCircular();
        ...
        for (int i = 0; i < numEntrada; i++) {
            for (String line : fileLines) {
                if (entradas[i].equals(line.split(",")[0])) {
                    ...
                    game.ler(line);
                    try {
                        gameQueue.inserir(game);
                        MyIO.println(gameQueue.media());
                    } catch (Exception e) {
                    }
                    break;
                }
        ...
        ArrayList<String> gamesRemoved = new ArrayList<String>();
        ...
        for (int i = 0; i < numEntrada; i++) {
            ...
            switch (comando_game[0]) {
                ...
                case "R":
                    try {
                        game = gameQueue.remover();
                        gamesRemoved.add(game.getName());
                        MyIO.println("R");
                        MyIO.println("(R) " + gamesRemoved.get(gamesRemoved.size() - 1));
                    } catch (Exception e) {
                    }
        ...
        gameQueue.mostrar();
        // for (int i = 0; i < gameQueue.tamanho(); i++) {
        //     MyIO.print("[" + i + "] ");
        //     gameQueue.get(i).imprimir();
        // }
    }
...
class GameFilaCircular {
    private Game[] array;
    private int primeiro, ultimo, tamanho;

    public GameFilaCircular() {
        this(5);
    }

    public GameFilaCircular(int tamanho) {
        this.array = new Game[tamanho + 1];
    ...
    public Game remover() throws Exception {
        ...
        Game game = this.array[this.primeiro];
