...
public class TP02Q06 {
    public static void main(String[] args) {
        ...
        Game game = new Game();
        GamePilha gameStack = new GamePilha();
        ...
        for (int i = 0; i < numEntrada; i++) {
            for (String line : fileLines) {
                if (entradas[i].equals(line.split(",")[0])) {
                    game = new Game();
                    game.ler(line);
                    gameStack.inserir(game);
                    break;
                }
            }
        }

        GamePilha gamesRemoved = new GamePilha();
        ...
        for (int i = 0; i < numEntrada; i++) {
            ...
            switch (comando_game[0]) {
                case "I":
                    ...
                    break;
                case "R":
                    gamesRemoved.inserir(gameStack.get(gameStack.tamanho() - 1));
                    gameStack.remover();
                    break;
                default:
                    break;
        ...
        for (int i = 0; i < gameStack.tamanho(); i++) {
            MyIO.print("[" + i + "] ");
            gameStack.get(i).imprimir();
        }
    }
...
class GamePilha {
    ...
    GamePilha() {
        this.gameStack = new Game[0];
        this.length = this.gameStack.length;
    }

    GamePilha(Game[] gameStack) {
        this.gameStack = gameStack;
        this.length = this.gameStack.length;
    }

    void inserir(Game game) {
        Game[] tmp = new Game[++this.length];
        tmp[this.length - 1] = game;
        for (int i = 0; i < this.length - 1; i++) {
            tmp[i] = this.gameStack[i];
        }
        this.gameStack = tmp;
        tmp = null;
    }
    ...
    void imprimir() {
        MyIO.print("[");
        if (this.length > 0)
            MyIO.print(this.gameStack[0].getName());
        for (int i = 1; i < this.length; i++)
            MyIO.print(", " + this.gameStack[i].getName());
        MyIO.println("]");
    }

    Game get(int pos) {
        return this.gameStack[pos];
    }

    void set(Game game, int pos) {
        this.gameStack[pos] = game;
    }
}
