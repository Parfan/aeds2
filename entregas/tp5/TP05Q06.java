...
public class TP05Q06 {
    public static void main(String[] args) {
        ...
        HashRehash gameHash = new HashRehash();
        ...
        while (isFim(entrada = sc.nextLine()) == false) {
			...
			int hPos = gameHash.getHashPosition(entrada);
			MyIO.println("Posicao: " + (hPos > 0 ? hPos : "NAO"));
        }
		sc.close();
    }
...
class HashRehash {
	...
	HashRehash(int tamTab) {
		this.tamTab = tamTab;
		tab = new Game[tamTab];
	}

	Game get(String s) {
		int pos = getHashPosition(s);
		return (pos > 0) ? tab[pos] : null;
	}
	...
	void put(Game game) {
		...
		if (tab[pos] == null) {
			tab[pos] = game;
		} else if (tab[(pos+1) % tamTab] == null) {
			tab[(pos+1) % tamTab] = game;
		}
	}

	int getHashPosition(String s) {
		int pos = h(s);
		if (tab[pos] != null && tab[pos].getName().equals(s)) {
			return pos;
		} else if (tab[(pos+1) % tamTab] != null && tab[(pos+1) % tamTab].getName().equals(s)) {
			return (pos+1) % tamTab;
		}
		return -1;
	}
	...
	private int h(String s) {
		int valorASCIINomeDoGame = 0;
		for (int i = 0; i < s.length(); i++) {
			valorASCIINomeDoGame += (int) s.charAt(i);
		}
		return valorASCIINomeDoGame % tamTab;
	}
}
