...
public class TP02Q04 {
    public static void main(String[] args) {
        ...
        while (isFim(entrada = MyIO.readLine()) == false) {
			i = runBinarySearchIteratively(gameList, entrada);
			if (i < 0) {
	...
	public static int runBinarySearchIteratively(ArrayList<Game> arr, String name) {
		int low = 0;
		int high = arr.size() - 1;
		
		while (low <= high) {
			int mid = low + ((high - low) / 2);
			if (arr.get(mid).getName().compareTo(name) < 0) {
				low = mid + 1;
	...
	public static void sort(ArrayList<Game> arr) {
	...
	public static void swap(ArrayList<Game> arr, int pos1, int pos2) {
		Game tmp = arr.get(pos1);
