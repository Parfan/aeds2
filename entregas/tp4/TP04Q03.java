...
public class TP04Q03 {
    public static void main(String[] args) {
        ...
        Game game = new Game();
        GameFilaCircular gameFila = new GameFilaCircular();
        ...
        for (int i = 0; i < numEntrada; i++) {
            for (String line : fileLines) {
                if (entradas[i].equals(line.split(",")[0])) {
                    game = new Game();
                    game.ler(line);
                    gameFila.inserir(game);
                    MyIO.println(gameFila.getAvg());
                    break;
                }
            }
        }
        ...
        for (int i = 0; i < numEntrada; i++) {
            ...
            switch (comando_game[0]) {
                case "I":
                    ...
                    MyIO.println(comando_game[0] + " " + comando_game[1]);
                    gameFila.inserir(game);
                    MyIO.println(gameFila.getAvg());
                    break;
                case "R":
                    MyIO.println(comando_game[0]);
                    MyIO.println("(R) " + gameFila.inicio.game.getName());
                    gameFila.remover();
                    break;
                default:
                    break;
        ...
        for (int i = 0; i < gameFila.tamanho(); i++) {
            MyIO.print("[" + i + "] ");
            gameFila.get(i).imprimir();
        }
    }
...
class GameFilaCircular {
    ...
    GameFilaCircular(int maxLength) {
        inicio = fim = null;
    ...
    void inserir(Game game) {
        if (length == 0) {
            inicio = new Celula(game);
            fim = inicio;
            length++;
        } else if (length < maxLength) {
            fim.prox = new Celula(game);
            fim = fim.prox;
            length++;
            if (length == maxLength) {
                fim.prox = inicio;
    ...
    int getAvg() {
        ...
        for (int i = 0; i < length; i++) {
            sum += cel.game.getAvg_pt();
            cel = cel.prox;
        }
        return (int) Math.round((sum * 1.0f) / length);
    }
