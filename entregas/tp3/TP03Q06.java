...
public class TP03Q06 {
	...
	public static void sortGameList(ArrayList<Game> array) {
		sortGameList(array, 0, array.size() - 1);
	}

    public static void sortGameList(ArrayList<Game> array, int esq, int dir) {
		int i = esq, j = dir;
        Game pivo = array.get((dir+esq)/2);

        while (i <= j) {
            while (array.get(i).getRelease_date().before(pivo.getRelease_date())) i++;
            while (array.get(j).getRelease_date().after(pivo.getRelease_date())) j--;
            if (i <= j) {
                swapGame(array, i, j);
                i++;
                j--;
        ...
        if (esq < j)
			sortGameList(array, esq, j);
        if (i < dir)
			sortGameList(array, i, dir);
	}

	public static void swapGame(ArrayList<Game> array, int i, int j) {
        Game tmp = array.get(i);
